...
description: 前端项目探索与报告生成技能。当用户要求分析一个前端项目的结构、生成项目架构报告、探索不熟悉的代码库、进行项目交接或架构摸底时触发。该技能会自动探索项目的路由结构、分析每个页面的功能与接口依赖、统计微服务使用情况、分析页面间跳转关系，最终输出一份完整的 `project_report.html` 项目报告。适用于：接到外包项目需要快速了解、项目文档缺失需要逆向梳理、新成员 onboarding、架构评审等场景。
---

# 前端项目分析报告 (FE Project Report)

探索前端项目并生成 `project_report.html` 报告。
...
# 生成的 project_report.html 会自动用浏览器打开
# 如需在无头环境中使用，添加 --no-open 参数
```
